...
(function () {
  ...
  function buildFigure(el, kind, title) {
    var html = '';
  ...
  function renderWar(el) {
    ...
    for (var i = 0; i < rows.length; i++) {
      var w = rows[i].war;
      if (w == null || !isFinite(parseFloat(w))) continue;
      kept.push(rows[i]);
    }
    if (!kept.length) { el.innerHTML = ''; return; }

    var minV = Infinity, maxV = -Infinity;
    for (i = 0; i < kept.length; i++) {
      var v = num(kept[i].war, 0);
      if (v < minV) minV = v;
      if (v > maxV) maxV = v;
    }
    ...
    // blank row after each player (including last)
    var rowGap = parseInt(el.getAttribute('data-row-gap') || String(rowH), 10);
    if (!isFinite(rowGap) || rowGap < 0) rowGap = rowH;

    var plotW = W - padL - padR;

    var plotHBars  = kept.length * rowH + Math.max(0, kept.length - 1) * rowGap;
    var plotHTotal = kept.length * (rowH + rowGap);
    ...
    function rect(x, y, w, h, cls) {
      if (w < 0) { x += w; w = -w; }
      return '<rect class="' + cls + '" x="' + x + '" y="' + y + '" width="' + w + '" height="' + h + '" rx="3" ry="3"/>';
    }
    function text(x, y, cls, str, anchor, extra) {
      return '<text class="' + cls + '" x="' + x + '" y="' + y + '" text-anchor="' + (anchor || 'start') + '"' + (extra || '') + '>' + str + '</text>';
    }

    var title = el.getAttribute('data-title') || 'Wins Above Replacement (WAR)';
    var plotHost = buildFigure(el, 'war', title);

    var svg = [];
    svg.push('<svg class="advsvg advsvg--war" viewBox="0 0 ' + W + ' ' + H + '" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="' + esc(title) + '">');
    ...
    svg.push('.warbar{fill:rgba(59,130,246,.60);stroke:rgba(255,255,255,.14);stroke-width:1;shape-rendering:geometricPrecision}');
    ...
    function showTip(hit, x, y) {
      ...
      html += '<div class="adv-tooltip__row"><span class="adv-tooltip__k">WAR</span><span class="adv-tooltip__v">' + esc(warV || '—') + '</span></div>';
  ...
  function boot(root) {
    root = root || document;
    var els = root.querySelectorAll('.js-adv-war');
    for (var i = 0; i < els.length; i++) renderWar(els[i]);
  }

  window.SJMS_WAR_BOOT = boot;
